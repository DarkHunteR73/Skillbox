...
void getStatus(int& state, int& hour)
{
    ...
    if (state & MAIN_SWITCH) std::cout << "POWER: on" << std::endl;
    else std::cout << "POWER: off" << std::endl;

    if (state & OUTLETS) std::cout << "OUTLETS: on" << std::endl;
    else std::cout << "OUTLETS: off" << std::endl;

    if (state & LIGHT_INDOOR) std::cout << "INDOOR LIGHT: " << lightTemp << std::endl;
    else std::cout << "LIGHT INDOOR: off" << std::endl;

    if (state & LIGHT_OUTDOOR) std::cout << "OUTDOOR LIGHT: on" << std::endl;
    else std::cout << "OUTDOOR LIGHT: off" << std::endl;

    if (state & BOILER) std::cout << "BOILER: on" << std::endl;
    else std::cout << "BOILER: off" << std::endl;

    if (state & WATERPIPE_HEATER) std::cout << "WATERPIPE HEATING: on" << std::endl;
    else std::cout << "WATERPIPE HEATING: off" << std::endl;

    if (state & AIR_CONDITIONING) std::cout << "A/C: on" << std::endl;
    else std::cout << "A/C: off" << std::endl;
...
Надо разработать упрощённую модель умного дома для дачи.Есть несколько датчиков : датчик температуры снаружи, датчик температуры внутри дома.
...
Первый автомат - это отключение и включение питания всего дома, второй - основные розетки дома, третий - свет внутри дома, четвёртый - наружный свет в саду,
пятый - отопление в помещении, шестой - отопление водопровода идущего из скважины с насосом, шестой - кондиционер в помещении.
...
Программа выполняется следующим образом.Каждый час пользователь сообщает состояние всех основных датчиков и света : температура снаружи, температура внутри,
есть ли движение снаружи, включен ли свет в доме.Данные параметры вводятся разом в одну строку, через пробел, а потом парсятся в переменные из
строкового буфера stringstream.Информация о

движении выводится в формате yes / no.Включение и отключение света происходит с помощью on / off.Стартовое время для симуляции умного дома - это 0 : 00.
