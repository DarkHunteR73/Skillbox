...
int main()
{
    ...
    while (expertsScore < 6 && tvWatchersScore < 6)
    {
        ...
        {
            std::cout << "\nThis is the correct answer. Experts team gets a point!" << std::endl;
            ++expertsScore;
        }
        else
        {
            std::cout << "\nThis is the wrong answer. TV watchers team gets a point!" << std::endl;
            ++tvWatchersScore;
        }

        std::cout << "\nExperts: " << expertsScore << ", TV watchers: " << tvWatchersScore << std::endl << std::endl;
    }

    std::cout << (expertsScore == 6 ? "Experts" : "TV watchers") << " team is win! Congratulations!" << std::endl;
}
...
void getQuestion(std::vector<bool>& vec, int& sector)
{
    ...
    question.open(".\\questions\\question" + std::to_string(sector + 1) + ".txt", std::ios::binary);
...
с клавиатуры вводится офсет (смещение) относительно текущего сектора на барабане. Исходя из этого офсета вычисляется новый активный сектор,
который и будет играть в этом ходе. Если выпавший сектор уже играл, выбирает следующий неигравший за ним. В начале всей игры стрелка установлена
на первом секторе.
