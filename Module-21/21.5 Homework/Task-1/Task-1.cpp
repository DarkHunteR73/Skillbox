...
int main()
{
    std::cout << "Select operation ('a' to add new entry, 'l' to list all entries):" << std::endl;
    char operation = ' ';
    while (operation != 'a' && operation != 'l')
    {
        ...
        if (operation == 'a')
        {
            ...
            while (true)
            {
                ...
                inEntry >> entry.firstName >> entry.lastName >> entry.date >> entry.salary;
            ...
            outPayroll << entry.firstName << ' ' << entry.lastName << ' ' << entry.date << ' ' << entry.salary << std::endl;
            outPayroll.close();
        }
        else if (operation == 'l')
        {
            ...
            else
            {
                while (!inPayroll.eof())
                {
                    ...
                    inPayroll >> entry.firstName >> entry.lastName >> entry.date >> entry.salary;

                    if (entry.firstName.empty())
                        continue;

                    std::cout << entry.firstName << ' ' << entry.lastName << ' ' << entry.date << ' ' << entry.salary << std::endl;
                }
            }
...
При старте программы пользователь указывает команду, которую следует осуществить: list или add. Команда read осуществляет чтение из файла 
ведомости как и прежде, только уже в структуры данных и отображает их на экране.
