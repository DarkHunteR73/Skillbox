...
{
    if (adress[0] == '.' || adress[adress.length() - 1] == '.') return false; //Если начало или конец '.', то false
    else
    {
        int dotCount = 0;
        for (int i = 0; i < adress.length(); ++i)
        {
            ...
            else
            {
                ...
                if ((adress[0] == '0' && adress[1] != '.')
                    || (adress[i] == '.' && adress[i + 1] == '.')
                    || (adress[i] == '.' && adress[i + 1] == '0'
                    && adress[i + 2] != '.' && i != adress.length() - 2)) return false; //Исключаем ведущие нули и двойные точки
            }
        }
        return dotCount == 3;
    }
